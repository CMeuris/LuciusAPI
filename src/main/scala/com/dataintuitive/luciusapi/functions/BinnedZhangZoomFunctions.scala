...
object BinnedZhangZoomFunctions extends Functions {
  ...
  def result(data:Input, par:Parameters) = {
    ...
    SeqMapStringAnyZoom(result.map{case (i, z, j, pwid) => Map("indexNew" -> i, "indexOld" -> j, "zhang" -> z, "pwid" -> pwid)})
